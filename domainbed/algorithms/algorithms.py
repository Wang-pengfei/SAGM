...
class GSAM_Fix(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.lr_scheduler = LinearScheduler(T_max=15000, max_value=self.hparams["lr"],
                                    min_value=self.hparams["lr"], optimizer=self.optimizer)

        self.rho_scheduler = LinearScheduler(T_max=15000, max_value=0.05,
                                         min_value=0.05)

        self.gsam_optimizer = GSAM(params=self.network.parameters(), base_optimizer=self.optimizer, model=self.network,
    ...
    def update(self, x, y, **kwargs):
        ...
        scale = self.hparams["rho"] / torch.cat([x.flatten() for x in grad_w]).norm(2)
        ...
        with torch.no_grad():
            for p, v in zip(self.network.parameters(), eps):
                p.add_(v)
        loss_p = F.cross_entropy(self.predict(all_x), all_y)


        with torch.no_grad():
            for p, v in zip(self.network.parameters(), eps):
                p.sub_(v)
        with open("gsam_0.txt", "a") as f:
            f.write(str(loss_p.item()))
            f.write("\n")
...
class ERM_flat(ERM):
    ...
    def update(self, x, y, **kwargs):
        ...
        grad_wad = torch.autograd.grad(loss2, self.network.parameters())
        grad_w = flat_grad(grad_w)
        grad_wad = flat_grad(grad_wad)
        S = torch.cosine_similarity(grad_w, grad_wad)
        # print(S)

        with open("erm_0.txt", "a") as f:
            f.write(str(S.item()))
            f.write("\n")
...
class SAMATE(ERM):
    ...
    def update(self, x, y, **kwargs):
        ...
        grad_w = autograd.grad(loss1, self.network.parameters())
        scale = self.hparams["rho"] / self.norm(grad_w)
        eps = [g * (scale-self.hparams["alpha"]) for g in grad_w]
        ...
        with open("sagm_0.txt", "a") as f:
            f.write(str(loss2.item()))
            f.write("\n")
        ###

        # 3. w = w - lr * g(w')
        loss = (F.cross_entropy(self.predict(all_x), all_y) + F.cross_entropy(inner_net(all_x), all_y))/2

        # loss = F.cross_entropy(self.predict(all_x), all_y)

        # grad_SAM = autograd.grad(loss, self.network.parameters())
        ...
        # with torch.no_grad():
        #     for p, v in zip(self.network.parameters(), grad_SAM):
        #         print(v)

        # with torch.no_grad():
        #     for p, v in zip(self.network.parameters(), grad_SAM):
        #         p.add_(v, alpha=-self.hparams["lr"])
...
class SAMATE_Mixstyle(Algorithm):
    """MixStyle w/o domain label (random shuffle)"""
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        # self.featurizer = networks.ResNet(input_shape, self.hparams, network=None, inset_mix=["conv2_x", "conv3_x", "conv4_x"])
        self.classifier = nn.Linear(self.featurizer.n_outputs, num_classes)
        self.network = nn.Sequential(self.featurizer, self.classifier)
        self.optimizer = self.new_optimizer(self.network.parameters())
    ...
    def update(self, x, y, **kwargs):
        ...
        # end mixstyle
        self.network[0].network.mixstyle_layers = []

        loss1 = F.cross_entropy(self.network(all_x), all_y)
        inner_net = copy.deepcopy(self.network)
        ...
        # 2. w' = w + eps(w)
        with torch.no_grad():
            for p, v in zip(self.network.parameters(), eps):# start mixstyle
            # self.network[
                p.add_(v)

        # start mixstyle
        # self.network[0].network.mixstyle_layers = ["conv2_x", "conv3_x", "conv4_x"]
        self.network[0].network.mixstyle_layers = ["conv2_x", "conv3_x"]


        # 3. w = w - lr * g(w')
        loss = (F.cross_entropy(self.predict(all_x), all_y) + F.cross_entropy(inner_net(all_x), all_y)) / 2

        # print(self.network[0].network.mixstyle_layers)
        # print(inner_net[0].network.mixstyle_layers)
...
class SAMATE_Mixstyle2(Algorithm):
    """MixStyle w/o domain label (random shuffle)"""
    ...
    def update(self, x, y, **kwargs):
        ...
        # 3. w = w - lr * g(w')
        loss = 0.8*F.cross_entropy(self.predict(all_x), all_y) + 0.2*F.cross_entropy(inner_net(all_x), all_y)
...
class SAMATE_2loss(ERM):
    ...
    def update(self, x, y, **kwargs):
        ...
        # 3. w = w - lr * g(w')
        loss = F.cross_entropy(self.predict(all_x), all_y) + F.cross_entropy(inner_net(all_x), all_y)
